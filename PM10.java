...
Влез: Во првиот ред од влезот е даден бројот на мерења N (N<=10 000), а во секој нареден ред е прво дадена населбата и концентрацијата на PM10 разделени со празно место. Во последниот ред е дадена населбата за која треба да најдете просечна концентрација на PM10 честички.
...
public class PM10 {
    public static void main(String[] args) throws IOException {
        ...
        if(koficka == -1){
            ...
            parts = vkupnoMerenja.split(" ");
            for(int i=0;i<parts.length;i++){
                rez += Double.parseDouble(parts[i]);
            }
        }
        rez = rez/parts.length;
        DecimalFormat df = new DecimalFormat("######.##");
